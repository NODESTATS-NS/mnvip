...
void PIVXGUI::createTrayIcon(const NetworkStyle* networkStyle)
{
    ...
    QString toolTip = tr("mnvip client") + " " + networkStyle->getTitleAddText();
...
{
    QString strTitle =  tr("mnvip"); // default title
...
bool PIVXGUI::openStandardDialog(QString title, QString body, QString okBtn, QString cancelBtn)
{
    ...
    if (isVisible()) {
        ...
        dialog->setWindowTitle(tr("mnvip"));
        dialog->adjustSize();
        dialog->raise();
        dialog->exec();
    }
